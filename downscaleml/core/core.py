...
def match_to_mid_resolution(
    source_ds: xr.Dataset,
    target_ds: xr.Dataset,
    lat_name: str = "lat",
    lon_name: str = "lon",
    num_mid_lats: Optional[int] = None,
    num_mid_lons: Optional[int] = None,
) -> xr.Dataset:
    ...
    extra_dims = {
        dim: source_mid.coords[dim]
        for dim in source_mid.dims
...
def encode_doys(
    ...
    def get_spatial_dims(ds):
        ...
        x_dim = next(
            (d for d in ["x", "lon", "longitude", "long", "lons"] if d in dims), None
        )
    ...
    sin_doy, cos_doy = (np.sin(2 * np.pi * doys / 365), np.cos(2 * np.pi * doys / 365))
