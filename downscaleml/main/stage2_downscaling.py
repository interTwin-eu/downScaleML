...
def main():
    ...
    try:
        ...
        if args.test_path:
            ...
            X_test_external = xr.open_zarr(args.test_path).sel(lat=slice(46, 47), lon=slice(10, 12)).compute()
            X_test_external = X_test_external[[args.target_var, "dem", "cos_doy", "sin_doy"]]
        ...
        if args.test_path:
            ...
            test_external_preds = np.full((len(X_test_external.time), len(X_train.y), len(X_train.x)), np.nan)
            
            # Progress bar for external test prediction
            with tqdm(total=len(models), desc="Predicting external test period") as pbar:
            ...
            test_external_out = output_dir / f'{args.target_var}_test_external_predictions.zarr'
            ...
            test_temporal_preds = np.full((len(X_test_temporal.time), len(X_train.y), len(X_train.x)), np.nan)
            
            # Progress bar for test prediction
            with tqdm(total=len(models), desc="Predicting temporal test period") as pbar:
                for (i,j), (model, scaler) in models.items():
            ...
            test_temporal_out = output_dir / f'{args.target_var}_test_temporal_predictions.zarr'
