...
def load_process_data(conn: LocalConnection, stac_url: str, 
                    spatial_extent: Dict[str, float],
                    temporal_extent: Optional[Tuple[str, str]] = None,
                    bands: Optional[list] = None) -> xr.Dataset:
...
def process_dem(conn: LocalConnection, spatial_extent: Dict[str, float], 
               use_xy_names: bool = False) -> xr.Dataset:
    ...
    dem_ds = dem.to_dataset(dim='bands')["dem"].to_dataset().isel(time=0).drop_vars("time")
    if use_xy_names:
...
def process_era5(output_dir: str, spatial_extent: Dict[str, float]) -> None:
    ...
    try:
        ...
        era5_interp.chunk({"time": 1000, "lat": 50, "lon": 50}).to_zarr(
            f"{output_dir}/ERA5_to_latent.zarr",
            mode="w",
            compute=True
        )
...
def process_seas5(output_dir: str, spatial_extent: Dict[str, float],
                 month_year: Optional[str] = None) -> None:
    ...
    try:
        ...
        for month in months:
            for year in years:
                ...
                if month in ["AUGUST", "SEPTEMBER", "OCTOBER", "NOVEMBER", "DECEMBER"] and year != "2021":
                    continue
                if month not in ["AUGUST", "SEPTEMBER", "OCTOBER", "NOVEMBER", "DECEMBER"] and year != "2022":
                    continue
                ...
                seas5_interp.chunk({"time": 1000, "lat": 50, "lon": 50, "number": 1}).to_zarr(
...
def main():
    ...
    parser.add_argument('--output-dir', type=str, default="/mnt/CEPH_PROJECTS/InterTwin/Climate_Downscaling/EMO1_DOWNSCALING/stage_1",
