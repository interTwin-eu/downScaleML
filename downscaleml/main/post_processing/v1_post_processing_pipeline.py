...
def main():
    ...
    logger.info(
        f"Setting up Dask cluster with {n_workers} workers and {threads_per_worker} threads per worker"
    )
    ...
    dem = local_conn.load_stac(
        url=stac_item, spatial_extent=spatial_extent, bands=["dem"]
    ).execute()
    ...
    base_paths = {
        "t2m": Path(
            "/mnt/CEPH_PROJECTS/InterTwin/Climate_Downscaling/EMO1_DOWNSCALING/stage_1/v3/t2m_v3"
        ),
        "tp": Path(
            "/mnt/CEPH_PROJECTS/InterTwin/Climate_Downscaling/EMO1_DOWNSCALING/stage_1/v3/tp_v3_1/tp"
        ),
        "ssrd": Path(
            "/mnt/CEPH_PROJECTS/InterTwin/Climate_Downscaling/EMO1_DOWNSCALING/stage_1/v3/ssrd_v3_1/ssrd"
        ),
    }
    ...
    output_dir = Path(
        "/mnt/CEPH_PROJECTS/InterTwin/Climate_Downscaling/PAPER/v1/SEAS5/"
    )
    output_dir.mkdir(parents=True, exist_ok=True)

    variable_units = {"t2m": "K", "tp": "mm/day", "ssrd": "J/m²/day", "pet": "mm/day"}
