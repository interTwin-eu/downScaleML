...
class CombinedLoss(nn.Module):
    def __init__(self, lambda_l1=10.0, lambda_l2=5.0, lambda_adv=1.0, lambda_gp=10.0):
    ...
    def compute_gradient_penalty(self, critic, real_samples, fake_samples):
        ...
        grad_outputs = torch.ones_like(critic_interpolates, device=real_samples.device).detach()  # ✅ Prevents NaNs
        gradients = grad(
    ...
    def forward(self, real_data, fake_data, critic, fake_preds, real_preds):
        ...
        g_loss_l2 = self.l2_loss(fake_data, real_data)
        ...
        g_loss = (
            ...
            self.lambda_l1 * g_loss_l1 +
            self.lambda_l2 * g_loss_l2 
        )
...
def train_esrgan(
    ...
    mlflow.set_experiment("128xloss_function_mod_extra_rrdb_tasmean_downscaling_adige_archi_change_mid_weights_solved_scheduler_fix_save")
...
if __name__ == '__main__':
    ...
    scheduler_G = torch.optim.lr_scheduler.StepLR(
        optimizer_G, 
        step_size=5,  # Reduce LR after 5 epochs
        gamma=0.8     # Multiply LR by 0.1
    )
