        ...
        def get_month_year_key(file_path):
            ...
            try:
                ...
                return datetime.strptime(
                    month_year_str.lower().capitalize().replace("_", " "), "%B %Y"
                )
...
def main():
    ...
    try:
        ...
        y = (
            ...
            .sel(lat=slice(42, 51), lon=slice(4, 16))
            .compute()
        )
        ...
        model_file = output_dir / f"{args.target_var}_two_stage_models.joblib"
        ...
        if args.use_pretrained and model_file.exists():
            ...
            if args.use_pretrained:
                logger.warning(
                    f"Pretrained model file not found at {model_file}, training new models"
                )

            # 2. Train two-stage models for each pixel
            logger.info("Training two-stage pixel models")
            models = {}
            ...
            with tqdm(total=total_pixels, desc="Training models") as pbar:
                for i in range(len(X_train.y)):
                    for j in range(len(X_train.x)):
                        ...
                        # Stage 1: Binary classifier for rain/no-rain
                        y_binary = (y_pixel >= 0.01).astype(int)
                        ...
                        binary_model.fit(X_pixel, y_binary)

                        # Stage 2: Regression model for rainy days
                        rainy_days = y_binary == 1
                        ...
                        reg_model.fit(X_pixel[rainy_days], y_pixel[rainy_days])
            ...
            # Save all models
            model_store = {"models": models, "y_coords": y_coords, "x_coords": x_coords}
        ...
        with tqdm(total=len(models), desc="Predicting test period") as pbar:
            for (i, j), model_dict in models.items():
                X_test_pixel = X_test.isel(y=i, x=j).to_array().values.T

                # Stage 1: Predict probability of rain
                rain_proba = model_dict["binary_model"].predict_proba(X_test_pixel)[
                    :, 1
                ]
                rain_mask = rain_proba >= 0.5

                # Stage 2: Predict precipitation amount for rainy days
                test_preds[rain_mask, i, j] = model_dict["reg_model"].predict(
                    X_test_pixel[rain_mask]
                )

                # Set non-rainy days to 0
                test_preds[~rain_mask, i, j] = 0
                pbar.update(1)
        ...
        test_out = output_dir / f"{args.target_var}_test_predictions.zarr"
        ...
        # 4. Process SEAS5 forecasts if provided
        if seas5_paths:
            logger.info(f"Processing {len(seas5_paths)} SEAS5 forecasts")
            for seas5_path in seas5_paths:
                month_year = Path(seas5_path).stem.split("_")[-2:]
                month_year = "_".join(month_year)

                seas5 = (
                    xr.open_zarr(seas5_path)
                    .sel(y=slice(42, 51), x=slice(4, 16))
                    .compute()
                )
                ...
                seas5_preds = np.full(
                    (len(seas5.time), len(seas5.number), num_y, num_x), np.nan
                )

                # Progress bar for SEAS5 prediction
                total_models = len(models) * len(seas5.number)
                with tqdm(total=total_models, desc=f"SEAS5 {month_year}") as pbar:
                    for m in range(len(seas5.number)):
                        for (i, j), model_dict in models.items():
                            ...
                            X_seas5 = seas5.isel(y=i, x=j, number=m).to_array().values.T

                            # Stage 1: Predict probability of rain
                            rain_proba = model_dict["binary_model"].predict_proba(
                                X_seas5
                            )[:, 1]
                            rain_mask = rain_proba >= 0.5

                            # Stage 2: Predict precipitation amount
                            seas5_preds[rain_mask, m, i, j] = model_dict[
                                "reg_model"
                            ].predict(X_seas5[rain_mask])

                            # Set non-rainy days to 0
                            seas5_preds[~rain_mask, m, i, j] = 0
                            pbar.update(1)

                # Save SEAS5 predictions
                seas5_ds = xr.Dataset(
                ...
                del seas5, seas5_ds, seas5_preds
                gc.collect()
        else:
