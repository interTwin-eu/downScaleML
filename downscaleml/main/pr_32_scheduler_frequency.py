...
class Generator(nn.Module):
    def __init__(self, in_channels=4, out_channels=1):
        ...
        # Residual blocks
        self.residual_blocks = nn.ModuleList([ResidualBlock(64) for _ in range(30)])
    ...
    def forward(self, x):
        ...
        for block in self.residual_blocks:
            x = block(x) + residual  # Each block adds the initial residual
        x = self.conv_out(x)
        x = x.squeeze(1)  # Remove channel dim to get [B, H, W]
        return x
...
class SSIMLoss(nn.Module):
    def __init__(self, data_range=1.0):
        ...
        self.ssim = SSIM(data_range=data_range, size_average=True, channel=1)  # Assuming single-channel output
...
class CombinedLoss(nn.Module):
    def __init__(self, lambda_l1=10.0, lambda_l2=5.0, lambda_adv=1.0, lambda_gp=10.0, lambda_freq=5.0):
    ...
    def frequency_loss(self, real_data, fake_data):
        ...
        real_fft = torch.fft.fft2(real_data)
        fake_fft = torch.fft.fft2(fake_data)
        
        real_amp = torch.abs(real_fft)
        fake_amp = torch.abs(fake_fft)

        return self.l1_loss(fake_amp, real_amp)
    ...
    def forward(self, real_data, fake_data, critic, fake_preds, real_preds):
        ...
        loss_batch["g_loss_adv"] = -torch.mean(fake_preds)
        loss_batch["d_loss_real"] = -torch.mean(real_preds)
        loss_batch["d_loss_fake"] = torch.mean(fake_preds)

        # Content Losses (L1 and L2)
        loss_batch["g_loss_l1"] = self.l1_loss(fake_data, real_data)
        loss_batch["g_loss_l2"] = self.l2_loss(fake_data, real_data)

        # Frequency Loss
        loss_batch["g_loss_freq"] = self.frequency_loss(real_data, fake_data)

        # Gradient Penalty for discriminator
        loss_batch["gradient_penalty"] = self.compute_gradient_penalty(critic, real_data.unsqueeze(1), fake_data.unsqueeze(1))

        # Generator total loss
        loss_batch["g_loss"] = (
            self.lambda_adv * loss_batch["g_loss_adv"] +
            self.lambda_l1 * loss_batch["g_loss_l1"] +
            self.lambda_l2 * loss_batch["g_loss_l2"] +
            self.lambda_freq * loss_batch["g_loss_freq"]  # Adding frequency loss
        )
        
        # Discriminator total loss
        loss_batch["d_loss"] = (
            loss_batch["d_loss_real"] + loss_batch["d_loss_fake"] + self.lambda_gp * loss_batch["gradient_penalty"]
        )
...
def train_esrgan(
    ...
    mlflow.set_experiment("pr_32_scheduler")
    
    # Create main save directory
    save_dir = f"/mnt/CEPH_PROJECTS/InterTwin/Climate_Downscaling/two_stage/models/pr/{datetime.now().strftime('%Y%m%d-%H%M%S')}"
    os.makedirs(save_dir, exist_ok=True)
    
    # Loss functions
    criterion = CombinedLoss(lambda_l1, lambda_l2, lambda_adv, lambda_gp).to(device)
    ...
    with mlflow.start_run():
        ...
        for epoch in range(num_epochs):
            ...
            for batch_idx, (input_data, target_data) in enumerate(dataloader_train):
                ...
                batch_step = batch_idx + epoch * total_batches
                ...
                for _ in range(n_critic):
                    ...
                    with autocast("cuda", enabled=False):
                        ...
                        loss_batch = criterion(target_data, fake_data, discriminator, fake_preds, real_preds)

                    scaler_D.scale(loss_batch["d_loss"]).backward()
                ...
                scaler_G.scale(loss_batch["g_loss"]).backward()
                scaler_G.unscale_(optimizer_G)
                torch.nn.utils.clip_grad_norm_(generator.parameters(), 1.0)
                scaler_G.step(optimizer_G)
                scaler_G.update()

                epoch_g_loss += loss_batch["g_loss"].item() * batch_size
                epoch_d_loss += loss_batch["d_loss"].item() * batch_size
                epoch_d_loss_real += loss_batch["d_loss_real"].item() * batch_size
                epoch_d_loss_fake += loss_batch["d_loss_fake"].item() * batch_size
                epoch_gradient_penalty += loss_batch["gradient_penalty"].item() * batch_size
                epoch_d_loss += loss_batch["d_loss"].item() * batch_size
                epoch_g_loss_adv += loss_batch["g_loss_adv"].item() * batch_size
                epoch_g_loss_l1 += loss_batch["g_loss_l1"].item() * batch_size
                epoch_g_loss_l2 += loss_batch["g_loss_l2"].item() * batch_size
                epoch_g_loss_freq += loss_batch["g_loss_freq"].item() * batch_size
                total_samples += batch_size
            
            # Calculate epoch metrics
            avg_g_loss = epoch_g_loss / total_samples
            avg_d_loss = epoch_d_loss / total_samples
            avg_d_loss_real = epoch_d_loss_real / total_samples
            avg_d_loss_fake = epoch_d_loss_fake / total_samples
            avg_gradient_penalty = epoch_gradient_penalty / total_samples
            avg_g_loss_adv = epoch_g_loss_adv / total_samples
            avg_g_loss_l1 = epoch_g_loss_l1 / total_samples
            avg_g_loss_l2 = epoch_g_loss_l2 / total_samples
            avg_g_loss_freq = epoch_g_loss_freq / total_samples
            ...
            if (epoch + 1) % save_samples_every == 0:
                ...
                fig = plot_meteo_samples(sample_input, sample_pred, sample_target, variable_type='pr', max_val=100)
            ...
            mlflow.log_metrics({
                ...
                "train_d_loss_real": avg_d_loss_real,
                "train_d_loss_fake": avg_d_loss_fake,
                "train_gradient_penalty": avg_gradient_penalty,
                "train_g_loss_adv": avg_g_loss_adv,
                "train_g_loss_l1": avg_g_loss_l1,
                "train_g_loss_l2": avg_g_loss_l2,
                "train_g_loss_freq": avg_g_loss_freq,
            ...
            scheduler_D.step(val_metrics['ssim'])
...
def plot_meteo_samples(input_seq, prediction, target, variable_type='pr', max_val=50, use_log_scale=True):
    """
    Universal meteorological visualization function tailored for precipitation.
    
    Args:
        input_seq: Input sequence [C, H, W] (C=4: [pr, dem, sin_doy, cos_doy])
        ...
        use_log_scale: Whether to use log-scale for precipitation visualization
    Returns:
    ...
    input_var = input_seq[0]  # [H, W] (precipitation)

    vmin, vmax = (0, max_val) if variable_type == 'pr' else (-max_val, max_val)

    if use_log_scale and variable_type == 'pr':
        input_var = np.log1p(input_var)  # Log transform for visualization
        prediction = np.log1p(prediction)
        target = np.log1p(target)
        vmax = np.log1p(max_val)
    ...
    # Log-transformed MAE/RMSE for better representation of extreme precipitation
    log_mae = np.mean(np.abs(np.expm1(prediction) - np.expm1(target)))
    log_rmse = np.sqrt(np.mean((np.expm1(prediction) - np.expm1(target))**2))

    metrics_text = "\n".join([
        f"MAE: {mae:.2f} {config['unit']}",
        f"RMSE: {rmse:.2f} {config['unit']}",
        f"SSIM: {ssim:.3f}",
        f"Log-MAE: {log_mae:.2f} mm",
        f"Log-RMSE: {log_rmse:.2f} mm",
    ...
    sns.histplot(np.expm1(target).flatten(), bins=30, kde=True, color='blue', ax=axs[5], label='True')
    sns.histplot(np.expm1(prediction).flatten(), bins=30, kde=True, color='red', ax=axs[5], label='Predicted')
    axs[5].set_xlabel('Precipitation (mm)')
    axs[5].set_ylabel('Frequency')
    axs[5].legend()
    axs[5].set_title('Precipitation Intensity Distribution')
...
if __name__ == '__main__':
    ...
    Era5_ds = Era5_ds.sel(y=slice(46.23, 46.77), x=slice(11.07, 11.6)).chunk(CHUNKS) # Just cut short this area for using the full domain
    Obs_ds = Obs_ds.sel(y=slice(46.23, 46.77), x=slice(11.07, 11.6)).chunk(CHUNKS)  # Just cut short this area for using the full domain
    Era5_ds = Era5_ds.fillna(0)
    Obs_ds = Obs_ds.fillna(0)
    Era5_ds = Era5_ds.astype(np.float32)
    
    log_precip_obs = np.log1p(Obs_ds)
    mean_obs = log_precip_obs.mean(dim="time")
    std_obs = log_precip_obs.std(dim="time")
    Obs_ds = (log_precip_obs - mean_obs) / std_obs

    log_precip_era5 = np.log1p(Era5_ds)
    mean_era5 = log_precip_era5.mean(dim="time")
    std_era5 = log_precip_era5.std(dim="time")
    Era5_ds = (log_precip_era5 - mean_era5) / std_era5
    ...
    generator.load_state_dict(torch.load("/home/sdhinakaran/consoldiated_downScaleML/downScaleML/downscaleml/main/generator_epoch_500.pth"))
    discriminator.load_state_dict(torch.load("/home/sdhinakaran/consoldiated_downScaleML/downScaleML/downscaleml/main/discriminator_epoch_500.pth"))
