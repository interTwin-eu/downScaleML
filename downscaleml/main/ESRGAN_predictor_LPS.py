...
mlflow.set_experiment("LPS_region_ESRGAN")
...
if __name__ == '__main__':
    ...
    Era5 = ERA5Dataset(ERA5_PATH.joinpath('LPS'), ERA5_PREDICTORS,
    ...
    if DEM:
        ...
        dem = search_files(DEM_PATH, '^LPS_DEM.nc$').pop()
    ...
    Era5_ds = Era5_ds.sel(y=slice(46.17, 47.1), x=slice(10.38, 12.45))
    Obs_ds = Obs_ds.sel(y=slice(46.17, 47.1), x=slice(10.38, 12.45))
    ...
    Era5_RESULT = Era5_ds.sel(time=RESULT_PERIOD)
    ...
    LogConfig.init_log(f'predictors_train shape : {predictors_train.shape}')
    LogConfig.init_log(f'predictand_train shape : {predictand_train.shape}')
    ...
    generator_model_path = "/home/sdhinakaran/downScaleML/downscaleml/main/mlartifacts/890247757373469855/851ab42d3f554e24b49b19b159d1e41a/artifacts/best_generator.pth"
                
    generator = torch.load(generator_model_path, map_location=device)
    LogConfig.init_log(f"Generator: {generator}")
    ...
    LogConfig.init_log(f"ERA_5_Results shape analyse: {Era5_RESULT}")
    # store predictions in xarray.Dataset
    predictions = xr.DataArray(data=prediction, dims=['time', 'y', 'x'], coords=dict(time=pd.date_range(Era5_RESULT.time.values[0],Era5_RESULT.time.values[-1], freq='D'), lat=Obs_valid.y, lon=Obs_valid.x))
    ...
    predictions.to_netcdf("/home/sdhinakaran/eurac/downScaleML-refactoring/downscaleml/main/LPS_ESRGAN_tasmean_2016_downscaled_to_emo1.nc")
