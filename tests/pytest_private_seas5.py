...
def test_seas5_processing_pipeline(dask_client, test_parameters):
    ...
    try:
        ...
        seas5_single = local_conn.load_stac(
            ...
            bands=test_parameters["bands"]["era5"]  # Using same bands as ERA5 for consistency
        )
        ...
        zarr_path = os.path.join(output_path, f"TEST_CUBE_SEAS5_{test_parameters['raster_stac']['seas5_uuid']}/TEST_CUBE_SEAS5_{test_parameters['raster_stac']['seas5_uuid']}.zarr")
...
def test_end_to_end_pixel_model_pipeline(dask_client, test_parameters):
    ...
    local_conn = LocalConnection("./")
    train_xy = local_conn.load_stac(
        url=STAC_URLS["train_xy"],
        spatial_extent=spatial_extent,
        temporal_extent=test_parameters["temporal"]
    ).execute().to_dataset(dim="bands")

    test_x = local_conn.load_stac(
    ...
    # Convert to numpy
    X_train = X_train.astype(np.float32).to_array().values  # shape: (features, time, y, x)
    ...
    test_ds = xr.Dataset({target_var: (('time', 'y', 'x'), test_preds)},
    ...
    seas5_ds = xr.Dataset({target_var: (('time', 'number', 'y', 'x'), seas5_preds)},
                          coords={
                              'time': test_x.time,
