...
def test_parameters():
    ...
    return {
        ...
        "bands": {"era5": ["t2m", "ssrd", "tp"], "pressure": ["t_850"], "dem": ["dem"]},
    }
...
def test_data_processing_pipeline(dask_client, test_parameters):
    ...
    try:
        ...
        resampled = merged_era5.resample_cube_spatial(
            datasets["dem"], method="bilinear"
        )
        ...
        assert all(
            b in result.data_vars
            for b in test_parameters["bands"]["era5"]
